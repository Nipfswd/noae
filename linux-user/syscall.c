 ...
 *  Linux syscalls
 * 
 *  Copyright (c) 2003 Noah Juopperi
 *
...
static long do_socketcall(int num, long *vptr)
{
    ...
    default:
        noae_log("Unsupported socketcall: %d\n", num);
        ret = -ENOSYS;
        break;
    }
    return ret;
...
static long do_ioctl(long fd, long cmd, long arg)
{
    ...
    for(;;) {
        if (ie->target_cmd == 0) {
            noae_log("Unsupported ioctl: cmd=0x%04lx\n", cmd);
            return -ENOSYS;
        }
    ...
    case TYPE_PTR:
        ...
        break;
    default:
        noae_log("Unsupported ioctl type: cmd=0x%04lx type=%d\n", cmd, arg_type[0]);
        ret = -ENOSYS;
        break;
    }
    return ret;
...
#error not supported
    ...
    default:
    unimplemented:
        noae_log("Unsupported syscall: %d\n", num);
        ret = -ENOSYS;
        break;
    }
 fail:
